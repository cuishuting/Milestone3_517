...
class ModelDumper:
    ...
    def dump_model(self,new_performance,best_param):
        ...
        file_name=[i for i in os.listdir('models&results') if i.startswith('pred_%s'%self.model_name)]
        ...
        if new_performance<prev_performance:
            if prev_performance!=99999:
                os.remove(os.path.join('models&results',file_name))
            ...
            with open('models&results/model_%s_%f.pkl'%(self.model_name,new_performance),'wb') as f:
                pickle.dump(new_model,f)
            tmp_df = pd.DataFrame(data=result, columns=['target'])
            tmp_df.index += 1
            tmp_df.index.name = 'id'
            tmp_df.to_csv('models&results/pred_%s_%f.csv' % (self.model_name,new_performance), index=True)